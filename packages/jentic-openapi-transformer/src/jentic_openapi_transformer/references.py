...
from typing import Any, Iterator, List, MutableMapping, MutableSequence, Tuple, cast
...
PathSeg = str | int
JSONPath = Tuple[PathSeg, ...]  # simple tuple of keys/indices


def walk_nodes(root: Any) -> Iterator[Tuple[JSONPath, Any, Any, Any]]:
    """
    Depth-first traversal yielding (path_to_parent, parent_obj, key_or_index, value).

    - For dicts: key_or_index is the dict key; value is the dict value.
    - For lists: key_or_index is the list index; value is the item.
    """
    stack: List[Tuple[JSONPath, Any]] = [((), root)]
    while stack:
        path, node = stack.pop()
...
def iter_url_fields(root: Any) -> Iterator[Tuple[JSONPath, MutableMapping[str, Any], str, str]]:
    ...
    """
    for path, parent, key, value in walk_nodes(root):
        if (
            isinstance(parent, MutableMapping)
            and isinstance(key, str)
            and isinstance(value, str)
            and key in _URL_KEYS_EXPLICIT
        ):
            yield path, parent, key, value
